---
name: herdr
description: "Control Herdr, a terminal multiplexer for coding agents. Use when the user explicitly mentions Herdr, or by default for subagent delegation/parallel work whenever HERDR_ENV=1 (see Default routing below). Requires HERDR_ENV=1."
---
...
## Default routing for subagent delegation

When `HERDR_ENV=1`, prefer Herdr over the plain `subagent` tool for delegated/parallel work: give each delegate its own pane instead of an invisible in-process run.

- One delegate → split a sibling pane (`pane split`), start an agent in it (`agent start`), send the task (`agent prompt ... --wait`).
- Several delegates in parallel → split one pane per delegate, start + prompt each, then `agent wait`/`agent read` each to collect results.
- Fall back to the `subagent` tool when `HERDR_ENV` is unset, or when the user asks for a lightweight/no-visible-pane run.
...
## Quick one-call delegation to pi.dev

For a single delegate running the `pi` agent kind, don't hand-roll split+start+prompt: `scripts/pi_pane.py`
does it in one call and prints one JSON line, same pattern as the driver scripts elsewhere.

```bash
python3 <skill-dir>/scripts/pi_pane.py delegate --prompt "..." [--direction right|down] [--no-wait]
```

`--wait` (default) blocks until the prompt settles and returns the pane's recent output inline.
`--no-wait` returns `pane_id`/`agent_name` immediately for real parallel fan-out — launch several,
then `python3 <skill-dir>/scripts/pi_pane.py collect --name NAME` each to read results later.
Use this path unless you need a non-`pi` kind or a layout Herdr control commands don't cover.
Leave the pane open when done — do not `pane close`/`tab close` it; the general rule below still applies.

## Claude Code subagents run in tabs

Inside Herdr a global `PreToolUse` hook denies the in-process `Agent` tool for Claude Code.
Spawn every delegate as an interactive claude session in its own labeled tab via `scripts/claude_pane.py`:

```bash
python3 <skill-dir>/scripts/claude_pane.py delegate --prompt "..." [--agent-type <name>] [--model <m>] [--no-wait]
```

`--agent-type` injects the body of `.claude/agents/<name>.md` (project, then home) via
`--append-system-prompt`. The child is told to write its final report to
`~/.claude/herdr-subagents/<name>/output.md`; the driver waits for a settled state, nudges an
idle child that hasn't written the file (max 3), harvests the file, and closes the tab on
success. `BLOCKED` or errors leave the tab open for inspection. For parallel fan-out use
`--no-wait` per delegate, then `claude_pane.py collect --name <name>` each.
